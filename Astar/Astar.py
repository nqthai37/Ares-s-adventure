...
def manhattan(stones):
    distance = 0
    for s in stones:
        temp = min(abs(s.point[0] - sx) + abs(s.point[1] - sy) for sx, sy in switches)
        distance += temp
    return distance
...
def main():
    ...
    set_value("maze.txt")
    algorithm, steps, weight, node_generated, path = A_star(player, stones)
